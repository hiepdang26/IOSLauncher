...
class SearchSuggestionUtil {
    fun getSuggestionProvider(context: Context): com.vhmsoft.launcherios26.features.suggestions.SuggestionProvider {
        val defaultSearchEngine = defaultSearchEngine(context)

        return with(defaultSearchEngine) {
            when {
                contains(Provider.QWANT.key, true) -> com.vhmsoft.launcherios26.features.suggestions.QwantProvider()
                else -> com.vhmsoft.launcherios26.features.suggestions.DuckDuckGoProvider()
            }
        }
    }
