...
import com.vhmsoft.launcherios26.core.executors.MainThreadExecutor
...
fun <T, A> ensureOnMainThread(creator: (A) -> T): (A) -> T {
    return { it ->
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            try {
                com.vhmsoft.launcherios26.core.executors.MainThreadExecutor().submit(Callable { creator(it) }).get()
