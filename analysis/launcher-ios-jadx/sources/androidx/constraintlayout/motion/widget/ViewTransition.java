...
public class ViewTransition {
    ...
    */
    ...
    private void parseViewTransitionTags(Context context, XmlPullParser xmlPullParser) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), j71.ViewTransition);
        int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int index = typedArrayObtainStyledAttributes.getIndex(i);
            if (index == j71.ViewTransition_android_id) {
                this.mId = typedArrayObtainStyledAttributes.getResourceId(index, this.mId);
            } else if (index == j71.ViewTransition_motionTarget) {
    ...
    private void updateTransition(MotionScene.Transition transition, View view) {
        ...
        if (keyFrames != null) {
            ...
            int i2 = 0;
            while (i2 < size) {
                Key key = keyFramesForView.get(i2);
                i2++;
                keyFrames2.addKey(key.mo0clone().setViewId(id));
            }
    ...
    public void applyTransition(ViewTransitionController viewTransitionController, MotionLayout motionLayout, int i, en enVar, View... viewArr) {
        if (this.mDisabled) {
            return;
        }
        ...
        if (i2 == 1) {
            for (int i3 : motionLayout.getConstraintSetIds()) {
                if (i3 != i) {
                    en constraintSet = motionLayout.getConstraintSet(i3);
                    for (View view : viewArr) {
                        zm zmVarL = constraintSet.l(view.getId());
                        zm zmVar = this.mConstraintDelta;
                        if (zmVar != null) {
                            ym ymVar = zmVar.h;
                            if (ymVar != null) {
                                ymVar.e(zmVarL);
                            }
                            zmVarL.g.putAll(this.mConstraintDelta.g);
                        }
                    }
                }
            }
        ...
        enVar2.e(enVar);
        for (View view2 : viewArr) {
            zm zmVarL2 = enVar2.l(view2.getId());
            zm zmVar2 = this.mConstraintDelta;
            if (zmVar2 != null) {
                ym ymVar2 = zmVar2.h;
                if (ymVar2 != null) {
                    ymVar2.e(zmVarL2);
                }
                zmVarL2.g.putAll(this.mConstraintDelta.g);
            }
        }
        motionLayout.updateState(i, enVar2);
        motionLayout.updateState(x51.view_transition, enVar);
        motionLayout.setState(x51.view_transition, -1, -1);
        MotionScene.Transition transition = new MotionScene.Transition(-1, motionLayout.mScene, x51.view_transition, i);
        for (View view3 : viewArr) {
            updateTransition(transition, view3);
        }
        motionLayout.setTransition(transition);
        motionLayout.transitionToEnd(new wc(this, viewArr, 20));
    }
    ...
    public Interpolator getInterpolator(Context context) {
        ...
        if (i == -1) {
            final fy fyVarC = fy.c(this.mDefaultInterpolatorString);
    ...
    public boolean matchesView(View view) {
        ...
        return this.mTargetString != null && (view.getLayoutParams() instanceof nm) && (str = ((nm) view.getLayoutParams()).Y) != null && str.matches(this.mTargetString);
    }
